...
const CentralOffice = require('../model/CentralOffice');
...
router.post('/create', verify, async (req, res) => {
    ...
    // //userdata
    const sorts = await CentralOffice.findOne({ sorts: req.body.sorts })
    if (sorts) return res.status(409).send({ "message": "Already in that position" });

    //Create new user
    const centralCommittee = new CentralOffice({
        nep: req.body.nep,
        eng: req.body.eng,
    ...
    try {
        ...
        if (centralCommitteeSave) {
            res.status(201).send({ "message": "You Have Successfully Added Central Committee" })
        }
    } catch (error) {
...
router.get('/', async (req, res) => {
    const allReport = await CentralOffice.find({});
    let data = [];
    let i = 1;
...
router.get('/single/:id', async (req, res) => {
    const centrlcommittee = await CentralOffice.findOne({ _id: req.params.id });
...
//Central Committee
router.put('/updateCentral', async (req, res) => {
    const centrlcommittee = await CentralOffice.findOne({ _id: req.body.id });
    if (!centrlcommittee) res.status(404).send({ "message": "Central Committee Was Not Found" });
    ...
    await CentralOffice.updateOne({ _id: req.body.id }, { $set: updateData });
...
router.get("/app_frontend", async (req, res) => {
    try {
      const allLeader = await CentralOffice.find().sort({ sort2: 1 });
...
router.delete('/single/:id', verify, async (req, res) => {
    ...
    try {
        ...
        //Select single page
        const centrlcommittee = await CentralOffice.findOne({ _id: req.params.id });
        if (!centrlcommittee) return res.status(404).send({ "message": "Page in not found" });
        ...
        const delone = await CentralOffice.findByIdAndDelete(req.params.id);
        if (!delone) res.status(404).send("Sorry no file found");

        res.status(200).send({ "message": "The committee has been deleted successfully" });
    } catch (error) {
        console.log(error);
    }
});
