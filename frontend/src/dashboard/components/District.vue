<template>
  <section>
    ...
    <div
      v-for="(data, index) in houseofrepresentative.allmember"
      :key="index"
    >
      ...
      <Houseofrepresentative
        :payload="{
          ...
          houseofrepresentative: data.title,
        }"
...
import Houseofrepresentative from "./Houseofrepresentative.vue";
export default {
  name: "District",
  data: () => ({
    loading: true,
  ...
  methods: {
    ...mapActions(["gethouseofrepresentativecount"]),
    totalCount(data) {
      this.totalMember = 0;
      this.totalGender = 0;
  ...
  computed: {
    ...mapGetters(["allProvinceonly", "allhouseofrepresentativecount"]),
  },
  async created() {
    ...
    this.houseofrepresentative = response.data;
    console.log(this.houseofrepresentative)
