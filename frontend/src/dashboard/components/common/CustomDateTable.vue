...
export default {
  name: "Events",
  props: ["title", "breadcrumbs", "slug", "allEvents"],
  components: {
    TitleBreadCrumb,
  },
  data: () => ({
  ...
  methods: {
    ...
    view(item) {
      ...
      this.$router.push(route).catch((error) => console.lgo(error));
    },
    edit(item) {
      let route = `/dashboard/editpostscustomdate/${item.category[0].slug}/${item._id}`;
      this.$router.push(route).catch((error) => console.lgo(error));
    },
  },
  created() {
