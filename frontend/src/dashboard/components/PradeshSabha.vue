<template>
  <section>
    <h4>
      {{ payload.maindistrict }} / {{ payload.houseofrepresentative }} /
      {{ payload.pradeshassemblyconstituency }}
    </h4>
    <div v-if="!loading">
    ...
    <div
      v-for="(data, index) in municipality.allmember"
      :key="index"
    >
      <Municipality
        :payload="{
          ...
          pradeshassemblyconstituency: payload.pradeshassemblyconstituency,
...
export default {
  ...
  methods: {
    ...mapActions(["getpradeshassemblyconstituencycount", "getmunicipality"]),
    totalCount(data) {
      this.totalMember = 0;
      this.totalGender = 0;
  ...
  computed: {
    ...mapGetters(["allProvinceonly", "allmuncipalitycount"]),
  },
  async created() {
    await this.reportGeneration({
      ...
      pradeshassemblyconstituency: this.payload.pradeshassemblyconstituency,
    });
