...
export default {
  ...
  computed: {
    ...
    ...mapGetters(["allDashCategories", "allCentral"]),
  },

  methods: {
    ...mapActions(["fetchCentralOffice"]),
    initialize() {
      this.desserts = this.allDashCategories.desserts;
    },
    editItem(item) {
      this.$router.push(`/dashboard/editcentraloffice/${item._id}`);
    },
    deleteItem(item) {
      if (confirm("Are you sure ?")) {
        this.$store.dispatch("deleteCentral", item._id);
        this.initialize();
      }
    },
