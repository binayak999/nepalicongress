  ...
  methods: {
    ...
    printCSV() {
      if (this.layout === "chart") {
        ...
        // Generate filename
        const filename = `${this.selectP[0].label}-${this.district.name}-${this.memberType !== 'Old' ? 'नयाँ' : 'नवीकरण'}-${this.houseofrepresentative.code}-${this.pradeshassemblyconstituency.code}.xlsx`;
    ...
    async filterActive() {
      ...
      if (this.houseofrepresentative.code != undefined) {
        filterData.houseofrepresentative = this.houseofrepresentative.code.toString();
      }
