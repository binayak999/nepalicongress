  ...
  <div>
    ...
    <div v-if="!loading">
      ...
      <section v-if="loggedActiveStorage || logged.user.role != 'subdomain'">
        ...
        <!-- <router-link
        to="/dashboard/addActiveMember"
        v-if="hasChetra != 'Yes' && logged.user.role != 'admin'"
      >
        <v-btn class="btn btn-default btn-add">New Active Member</v-btn>
      </router-link> -->
        <!-- <router-link to="/dashboard/addRenewActiveMember"
      v-if="hasChetra == 'Yes' && logged.user.role != 'admin'"><v-btn class="btn btn-default btn-add">Renew Active Member</v-btn></router-link
      >-->
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      // { text: "Renew", value: "renew", sortable: false, width: "20%" },
    ],
    desserts: [],
    editedIndex: -1,
    editedItem: {
