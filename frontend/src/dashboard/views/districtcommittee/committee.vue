...
export default {
  ...
  computed: {
    ...
    ...mapGetters(["allDashCategories", "allDisCommittee"]),
  },

  methods: {
    ...mapActions(["fetchDisCommittee"]),
    initialize() {
      this.desserts = this.allDashCategories.desserts;
    },
    editItem(item) {
      this.$router.push(`/dashboard/editworkingcommittee/${item._id}`);
    },
    deleteItem(item) {
        if (confirm("Are you sure ? You want to delete")) {
      this.$store.dispatch("deleteDisCommittee",item._id);
      this.initialize();
        }
    },
