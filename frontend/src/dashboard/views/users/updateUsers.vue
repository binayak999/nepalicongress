...
export default {
  ...
  methods: {
    ...mapActions(["subdomainDashboardSingle"]),
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.$store.dispatch("updateSubdomainBySubdomain", auth);
      }
    },
    removeImage(data) {
  ...
  computed: {
    ...mapGetters(["subdomainSingle"]),
  },
  created() {
    this.subdomainDashboardSingle(
      JSON.parse(secureStorage.getItem("userData")).user.userHandle
    );
    ...
    setTimeout(() => {
      ...
      this.parentSubdomain = this.subdomainSingle.address;
      if (this.parentSubdomain != "" || this.parentSubdomain != undefined) {
        this.hasparent = true;
      }
