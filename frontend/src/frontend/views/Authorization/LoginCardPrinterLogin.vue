<template>
  <section>
    ...
    <v-container fluid v-if="logged">
      <v-btn @click="logout"  style="margin-left:115px" class="primary">Logout</v-btn>
      <ActiveMemberReport />
    </v-container>
    <v-container fluid v-if="!logged">
      <v-row class="loginbox">
        <v-col md="5" class="authbox">
          <h2>Login To Card Print</h2>
          <div>
            <v-form
              ref="formlogin"
              v-model="validTop"
...
import ActiveMemberReport from "@/dashboard/views/circular/cardprinters";
export default {
  name: "LoginCardPrinterLogin",
  components: {
    TitleBreadCrumb,
    ActiveMemberReport
  },
  ...
  methods: {
    ...
    async login() {
      if (this.$refs.formlogin.validate()) {
        ...
        await this.loginCardUser(auth);
        this.logged = secureStorage.getItem("cardUserData");
      }
    },
    logout() {
