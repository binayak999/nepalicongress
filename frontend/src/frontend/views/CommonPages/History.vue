...
export default { 
  ...
  computed: {
    ...mapGetters(["allHistoryFrontend", "selectedLanguage"]),
    ...
    numberOfPage: function () {
      if (this.allHistoryFrontend) {
        return Math.ceil(this.allHistoryFrontend.length / this.rows);
      } else {
        return 0;
      }
    },
    paged: function () {
      if (this.allHistoryFrontend) {
        return this.allHistoryFrontend.slice(
  ...
  methods: {
    ...mapActions(["fetchHistoryFrontend"]),
  },
  created() {
    this.fetchHistoryFrontend();
