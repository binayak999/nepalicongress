  ...
  <section>
    ...
    <v-container fluid>
      <v-row>
        ...
        <v-col md="9" class="boxsingle" v-if="paged.length != 0 && !loading">
          <div
...
export default {
  ...
  computed: {
    ...mapGetters(["allCircularDocumentFrontend", "selectedLanguage"]),
    ...
    numberOfPage: function() {
      if (this.allCircularDocumentFrontend) {
        return Math.ceil(this.allCircularDocumentFrontend.length / this.rows);
      } else {
        return 0;
      }
    },
    paged: function() {
      if (this.allCircularDocumentFrontend) {
        return this.allCircularDocumentFrontend.slice(
  ...
  methods: {
    ...mapActions(["fetchCircularDocumentFrontend"]),
  },
  async created() {
    await this.fetchCircularDocumentFrontend();
    this.loading = false;
