       ...
       <v-col md="9" class="boxsingle" v-if="paged.length == 0">
        ...
        <v-col md="9" class="boxsingle" v-if="paged.length != 0">
          <div
            ...
            :key="index"
          >
            <ListImageBig :data="list" :language="selectedLanguage.title" v-if="list.images.length !== 0" slug="rules-and-regulations"/>
            <ListBig :data="list" :language="selectedLanguage.title" v-if="list.images.length === 0" slug="rules-and-regulations"/>
          </div>
          <v-pagination
            v-model="page"
            :length="numberOfPage"
...
export default {
  ...
  computed: {
    ...mapGetters(["allRulesandRegulationFrontend", "selectedLanguage"]),
    ...
    numberOfPage: function () {
      if (this.allRulesandRegulationFrontend) {
        return Math.ceil(this.allRulesandRegulationFrontend.length / this.rows);
      } else {
        return 0;
      }
    },
    paged: function () {
      if (this.allRulesandRegulationFrontend) {
        return this.allRulesandRegulationFrontend.slice(
  ...
  methods: {
    ...mapActions(["fetchRulesNregulationFrontend"]),
  },
  created() {
    this.fetchRulesNregulationFrontend();
