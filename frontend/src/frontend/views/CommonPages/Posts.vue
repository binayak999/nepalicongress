...
export default {
  ...
  computed: {
    ...mapGetters(["allPostFrontend", "selectedLanguage"]),
    ...
    numberOfPage: function () {
      if (this.allPostFrontend) {
        return Math.ceil(this.allPostFrontend.length / this.rows);
      } else {
        return 0;
      }
    },
    paged: function () {
      if (this.allPostFrontend) {
        return this.allPostFrontend.slice(
  ...
  methods: {
    ...mapActions(["fetchPostsFrontend"]),
  },
  created() {
    this.fetchPostsFrontend();
