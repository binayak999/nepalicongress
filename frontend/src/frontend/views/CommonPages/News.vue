...
export default {
  ...
  computed: {
    ...mapGetters(["allNewsFrontend", "selectedLanguage"]),
    ...
    numberOfPage: function () {
      if (this.allNewsFrontend) {
        return Math.ceil(this.allNewsFrontend.length / this.rows);
      } else {
        return 0;
      }
    },
    paged: function () {
      if (this.allNewsFrontend) {
        return this.allNewsFrontend.slice(
  ...
  methods: {
    ...mapActions(["fetchNewsFrontend"]),
  },
  created() {
    this.fetchNewsFrontend();
