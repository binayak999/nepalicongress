...
export default { 
  ...
  computed: {
    ...mapGetters(["allOurInspirationFrontend", "selectedLanguage"]),
    ...
    numberOfPage: function () {
      if (this.allOurInspirationFrontend) {
        return Math.ceil(this.allOurInspirationFrontend.length / this.rows);
      } else {
        return 0;
      }
    },
    paged: function () {
      if (this.allOurInspirationFrontend) {
        return this.allOurInspirationFrontend.slice(
  ...
  methods: {
    ...mapActions(["fetchInspirationFrontend"]),
  },
  created() {
    this.fetchInspirationFrontend();
