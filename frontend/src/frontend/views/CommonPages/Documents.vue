...
export default {
  ...
  computed: {
    ...mapGetters(["allDocumentsFrontend", "selectedLanguage"]),
    ...
    numberOfPage: function() {
      if (this.allDocumentsFrontend) {
        return Math.ceil(this.allDocumentsFrontend.length / this.rows);
      } else {
        return 0;
      }
    },
    paged: function() {
      if (this.allDocumentsFrontend) {
        return this.allDocumentsFrontend.slice(
  ...
  methods: {
    ...mapActions(["fetchDocumentFrontend"]),
    seachData() {
      this.$store.dispatch("searchDocument", this.searchtext);
    },
  },
  async created() {
    await this.fetchDocumentFrontend();
    this.loading = false;
