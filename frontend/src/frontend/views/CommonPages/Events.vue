       ...
       <v-col md="9" class="boxsingle" v-if="paged.length == 0">
        ...
        <v-col md="9" class="boxsingle" v-if="paged.length != 0">
          <div
            ...
            :key="index"
          >
            <ListImageBig :data="list" :language="selectedLanguage.title" v-if="list.images.length !== 0" slug="partycontitution"/>
            <ListBig :data="list" :language="selectedLanguage.title" v-if="list.images.length === 0" slug="partycontitution"/>
          </div>
          <v-pagination
            v-model="page"
            :length="numberOfPage"
...
export default {
  ...
  computed: {
    ...mapGetters(["allEventsFrontend", "selectedLanguage"]),
    ...
    numberOfPage: function () {
      if (this.allEventsFrontend) {
        return Math.ceil(this.allEventsFrontend.length / this.rows);
      } else {
        return 0;
      }
    },
    paged: function () {
      if (this.allEventsFrontend) {
        return this.allEventsFrontend.slice(
  ...
  methods: {
    ...mapActions(["fetchEventsFrontend"]),
  },
  created() {
    this.fetchEventsFrontend();
